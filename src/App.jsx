...
const INITIAL_CATALOG = [
  ...
  { id:"q6",  cat:"定量·HUT留置", name:"额外：每增加1周使用",                    unitPrice:25,   calcType:"hut", notes:"叠加在基础款" },
  { id:"q7",  cat:"定量·HUT留置", name:"额外：每增加1份回访问卷",                 unitPrice:50,   calcType:"hut", notes:"" },
  { id:"q8",  cat:"定量·HUT留置", name:"额外：每增加1个留置产品",                 unitPrice:20,   calcType:"hut", notes:"" },
  ...
  { id:"ql6", cat:"定性研究",     name:"定性额外：每增加1周产品使用",              unitPrice:25,   calcType:"per-person", notes:"" },
  { id:"ql7", cat:"定性研究",     name:"定性额外：每增加1份回访问卷",              unitPrice:50,   calcType:"per-person", notes:"" },
  { id:"ql8", cat:"定性研究",     name:"定性额外：每增加1个留置产品",              unitPrice:20,   calcType:"per-person", notes:"" },
...
function wkStr(days){ const w=days/7; return Number.isInteger(w)?`${w}周`:`${w}周（${days}天）`; }

// 招募周期计算：最少0.5周，sampleSizeTotal/60 × 难度系数，取整到0.5周
...
// Topline天数：1-2产品=7天，每增加2产品+3.5天（从第3产品起）
function calcToplineDays(products){
  return 7 + Math.floor((products - 1) / 2) * 3.5;
}
// 报告天数：1产品=7天，每增加1产品+3.5天
function calcReportDays(products){
  return 7 + (products - 1) * 3.5;
}
...
export default function App() {
  ...
  const [params, setParams] = useState({
    title:              "HUT 留置测试",
    products:           1,
    samplePerProduct:   45,   // 单产品样本量（HUT计价用）
    sampleTotal:        45,   // 总样本量（CLT/定性/技术类用）
    sessions:           2,
    weeksDuration:      1,
    courierDays:        2,    // 快递时间（天）
    difficulty:         "normal",
    vatRate:            6,
  ...
  const [lines, setLines] = useState([
    { lid:"q1", qtyOverride:null, priceOverride:null },
    { lid:"d3", qtyOverride:null, priceOverride:null },
    { lid:"d1", qtyOverride:null, priceOverride:null },
    { lid:"d2", qtyOverride:null, priceOverride:null },
  ]);
  ...
  const tlData = useMemo(() => {
    ...
    const toplineDays = calcToplineDays(params.products);
    const reportDays  = calcReportDays(params.products);

    const recruitEnd = skipAdd(start, recruit.days);
    ...
    const topEnd     = skipAdd(topStart, toplineDays);
    const repStart   = skipWE(topEnd);
    const repEnd     = skipAdd(repStart, reportDays);

    return {
      recruit, leaveDays, toplineDays, reportDays,
      rows: [
        { phase:"项目确认 / PO",   days:"/",  start:"—",           end:fmtD(start),    note:"" },
        { phase:"甄别问卷确认",     days:"/",  start:"—",           end:fmtD(start),    note:"" },
        { phase:"招募",           days:`建议 ${recruit.min}~${recruit.max}周`, start:fmtD(start), end:fmtD(recruitEnd), note:`按最短${recruit.min}周估算` },
        { phase:"提供 Attributes", days:"/",  start:"—",           end:fmtD(skipWE(addDays(recruitEnd,-2))), note:"招募结束前" },
        { phase:"产品到达",        days:"/",  start:"—",           end:fmtD(skipWE(addDays(recruitEnd,-2))), note:"招募结束前" },
        ...
        { phase:"Topline",       days:toplineDays, start:fmtD(topStart), end:fmtD(topEnd), note:toplineDays===7?"1周":wkStr(toplineDays) },
        { phase:"Report",        days:reportDays,  start:fmtD(repStart), end:fmtD(repEnd),  note:wkStr(reportDays) },
      ]
    };
  }, [params]);

  // ── Catalog CRUD ──
  const updateCat = (id,k,v) => setCatalog(arr=>arr.map(c=>c.id===id?{...c,[k]:v}:c));
  const deleteCat = (id) => { setCatalog(a=>a.filter(c=>c.id!==id)); setLines(ls=>ls.filter(l=>l.lid!==id)); };
  const addCatItem = () => { _nid++; setCatalog(a=>[...a,{id:`x${_nid}`,cat:"交付与通用",name:"新费用项",unitPrice:0,calcType:"fixed",notes:""}]); };
  const addLine = (catId) => { if(lines.find(l=>l.lid===catId)) return; setLines(ls=>[...ls,{lid:catId,qtyOverride:null,priceOverride:null}]); };
  const removeLine = (lid) => setLines(ls=>ls.filter(l=>l.lid!==lid));
  const updateLine = (lid,k,v) => setLines(ls=>ls.map(l=>l.lid===lid?{...l,[k]:v===''?null:+v}:l));
  ...
  const exportXLSX = () => {
    ...
    const ws1 = XLSX.utils.aoa_to_sheet([
      [params.title], [],
      [`产品数: ${params.products}  单产品样本: ${params.samplePerProduct}人  总样本: ${params.sampleTotal}人  场次: ${params.sessions}  留置: ${params.weeksDuration}周  难度: ${DIFF[params.difficulty].label}`],
    ...
    <div style={{fontFamily:"'Noto Serif SC',Georgia,serif",background:"#f4f1ec",minHeight:"100vh",color:"#1a1a1a"}}>
      ...
      <div style={{background:"#2c2825",padding:"18px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
        <div>
          ...
          <input value={params.title} onChange={e=>setP("title",e.target.value)} style={titleInputStyle} />
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"272px 1fr",gap:18,padding:"18px 28px",maxWidth:1440,margin:"0 auto"}}>
        ...
        <div>
          <div style={S.card}>
            ...
            <F label="产品数量">
              <select className="inp" value={params.products} onChange={e=>setP("products",+e.target.value)}>
                {[1,2,3,4,5,6].map(n=><option key={n} value={n}>{n} 款</option>)}
              </select>
            </F>
            <Sep label="样本量" />
            <F label="单产品样本量（HUT计价用）">
              <input className="inp" type="number" min={1} value={params.samplePerProduct} onChange={e=>setP("samplePerProduct",+e.target.value)} />
            </F>
            <F label="总样本量（CLT / 定性 / 技术类）">
              <input className="inp" type="number" min={1} value={params.sampleTotal} onChange={e=>setP("sampleTotal",+e.target.value)} />
            </F>
            <Sep label="研究设置" />
            <F label="定性场次（组 / 场）">
              <input className="inp" type="number" min={1} value={params.sessions} onChange={e=>setP("sessions",+e.target.value)} />
            </F>
            <F label="留置时长">
              <select className="inp" value={params.weeksDuration} onChange={e=>setP("weeksDuration",+e.target.value)}>
                {[1,2,3,4].map(n=><option key={n} value={n}>{n} 周（{n*7}天）</option>)}
              </select>
            </F>
            <F label="快递时间（天）">
              <input className="inp" type="number" min={0} max={7} value={params.courierDays} onChange={e=>setP("courierDays",+e.target.value)} />
            </F>
            <F label="招募难易程度">
              <select className="inp" value={params.difficulty} onChange={e=>setP("difficulty",e.target.value)}>
                {Object.entries(DIFF).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
              </select>
            </F>
            <F label="VAT 税率 (%)">
              <input className="inp" type="number" min={0} max={20} value={params.vatRate} onChange={e=>setP("vatRate",+e.target.value)} />
            </F>
            <F label="项目确认日期" last>
              <input className="inp" type="date" value={params.startDate} onChange={e=>setP("startDate",e.target.value)} />
            </F>
          </div>

          {/* 参数预估摘要 */}
          <div style={{background:"#fff",borderRadius:8,padding:"14px 16px",marginBottom:0,border:"1px solid #e8e0d4",fontSize:12}}>
            <div style={{fontWeight:700,color:"#7a6e5f",fontSize:11,letterSpacing:"0.06em",marginBottom:10,textTransform:"uppercase"}}>时间表预估</div>
            {[
              ["招募周期", `建议 ${tlData.recruit.min}~${tlData.recruit.max} 周`],
              ["派发+留置", `${params.weeksDuration*7}天留置 + ${params.courierDays}天快递 = ${tlData.leaveDays}天`],
              ["Topline", wkStr(tlData.toplineDays)],
              ["Report", wkStr(tlData.reportDays)],
            ].map(([k,v])=>(
            ...
            <div style={{marginTop:10,fontSize:10.5,color:"#aaa",lineHeight:1.8}}>
              招募: 总样本 {params.sampleTotal}人 × 难度系数 {DIFF[params.difficulty].timeFactor}<br/>
              HUT计价: {params.samplePerProduct} × {params.products}款 × {DIFF[params.difficulty].priceMult}
            </div>
          </div>
        </div>
          ...
          <div style={{display:"flex",gap:3,marginBottom:12}}>
            {[["quote","📋 报价明细"],["catalog","📚 费用目录"],["timeline","📅 时间表"]].map(([id,label])=>(
              <button key={id} onClick={()=>setTab(id)}
                style={tab===id?S.tabOn:S.tabOff} className={tab===id?"tab-on":"tab-off"}>{label}</button>
            ))}
          </div>
            ...
            <div>
              <div style={S.card}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  ...
                  <div style={{fontSize:11,color:"#bbb"}}>前往「费用目录」tab 点「＋」添加</div>
                </div>
                <table style={S.table}>
                  <thead><tr>
                    <th style={{...S.th,width:"36%"}}>费用项目</th>
                    <th style={{...S.th,width:"14%"}}>计算方式</th>
                    <th style={{...S.th,width:"12%",textAlign:"right"}}>自动数量</th>
                    <th style={{...S.th,width:"11%",textAlign:"right"}}>覆盖数量</th>
                      ...
                      <tr key={r.lid} style={i%2===1?{background:"#faf8f5"}:{}}>
                        ...
                        <td style={{...S.td,textAlign:"right"}}>
                          <input type="number" placeholder="—"
                            value={r.qtyOverride??""} onChange={e=>updateLine(r.lid,"qtyOverride",e.target.value)}
                            style={{...S.mini,width:60,textAlign:"right"}} title="留空=自动" />
                        </td>
                        <td style={{...S.td,textAlign:"right"}}>
                          <input type="number" placeholder={r.ci.unitPrice}
                            value={r.priceOverride??""} onChange={e=>updateLine(r.lid,"priceOverride",e.target.value)}
                            style={{...S.mini,width:76,textAlign:"right"}} title="留空=目录价" />
                        </td>
                        <td style={{...S.td,textAlign:"right",fontFamily:"'DM Mono',monospace",fontWeight:600}}>{fmtRMB(r.total)}</td>
                        <td style={{...S.td,textAlign:"center"}}>
                          <button onClick={()=>removeLine(r.lid)} style={S.delBtn} className="del-btn">×</button>
                        </td>
                      </tr>
                    ))}
                    {lineRows.length===0&&<tr><td colSpan={7} style={{...S.td,color:"#ccc",textAlign:"center",padding:"28px 0"}}>尚未添加任何费用项</td></tr>}
                  </tbody>
                </table>
                  ...
                  <table style={{borderCollapse:"collapse",minWidth:340}}>
                    <tbody>
                      <tr style={{background:"#f0ebe3"}}>
                        ...
                        <td style={{...S.td,textAlign:"right",fontFamily:"'DM Mono',monospace",fontWeight:700,paddingRight:16,width:130}}>{fmtRMB(sub)}</td>
                      </tr>
              ...
              <div style={{background:"#fffbf5",border:"1px solid #ece3d0",borderRadius:6,padding:"10px 14px",fontSize:11,color:"#9a8e80",lineHeight:1.9}}>
                💡 「覆盖数量」「单价」留空 = 按参数自动计算 &nbsp;|&nbsp; 输入任意值则以输入为准，不影响其他行
              </div>
            </div>
          )}
                ...
                return (
                  <div key={cat} style={{marginBottom:22}}>
                    <div style={{fontSize:11,fontWeight:700,letterSpacing:"0.06em",color:"#7a6e5f",padding:"6px 10px",background:"#f0ebe3",borderRadius:4,marginBottom:6,display:"flex",justifyContent:"space-between"}}>
                      <span>{cat}</span>
                      {cat==="定量·HUT留置"&&<span style={{color:"#b08060",fontSize:10}}>计价 = 单产品样本量 × 产品数 × 难度系数</span>}
                    </div>
                    <table style={S.table}>
                      <thead><tr>
                        <th style={{...S.th,width:"32%"}}>名称</th>
                        <th style={{...S.th,width:"11%"}}>分类</th>
                        <th style={{...S.th,width:"12%",textAlign:"right"}}>单价</th>
                        <th style={{...S.th,width:"14%"}}>计算方式</th>
                        <th style={{...S.th,width:"12%"}}>备注</th>
                        <th style={{...S.th,width:"10%",textAlign:"center"}}>加入报价</th>
                        ...
                        {items.map((ci,i)=>{
                          ...
                          return (
                            <tr key={ci.id} style={i%2===1?{background:"#faf8f5"}:{}}>
                              ...
                              <td style={S.td}>
                                <select value={ci.cat} onChange={e=>updateCat(ci.id,"cat",e.target.value)} style={{...S.inlineInput,fontSize:10,padding:"3px 4px"}}>
                                  {CATS.map(c=><option key={c} value={c}>{c}</option>)}
                                </select>
                              </td>
                              <td style={{...S.td,textAlign:"right"}}>
                                <input type="number" value={ci.unitPrice} onChange={e=>updateCat(ci.id,"unitPrice",+e.target.value)} style={{...S.inlineInput,width:76,textAlign:"right",fontFamily:"'DM Mono',monospace"}}/>
                              </td>
                              <td style={S.td}>
                                <select value={ci.calcType} onChange={e=>updateCat(ci.id,"calcType",e.target.value)} style={{...S.inlineInput,fontSize:10,padding:"3px 4px"}}>
                              ...
                              <td style={{...S.td,textAlign:"center"}}>
                                {inQ
                                  ? <span style={{fontSize:11,color:"#27ae60",fontWeight:700}}>✓ 已选</span>
                                  : <button onClick={()=>addLine(ci.id)} style={S.addBtn} className="add-btn">＋</button>}
                              </td>
                              <td style={{...S.td,textAlign:"center"}}>
                                <button onClick={()=>deleteCat(ci.id)} style={S.delBtn} className="del-btn">×</button>
                              </td>
                            </tr>
                          );
                        })}
            ...
            <div style={S.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                ...
                <div style={{display:"flex",gap:16,fontSize:11,color:"#888"}}>
                  <span>招募建议 <b style={{color:"#2c2825"}}>{tlData.recruit.min}~{tlData.recruit.max}周</b></span>
                  <span>Topline <b style={{color:"#2c2825"}}>{wkStr(tlData.toplineDays)}</b></span>
                  <span>Report <b style={{color:"#2c2825"}}>{wkStr(tlData.reportDays)}</b></span>
                </div>
              </div>
              <table style={S.table}>
                <thead><tr>
                  <th style={{...S.th,width:"30%"}}>阶段</th>
                  <th style={{...S.th,width:"17%"}}>时间（建议）</th>
                  <th style={{...S.th,width:"18%",textAlign:"center"}}>开始日期</th>
                  <th style={{...S.th,width:"18%",textAlign:"center"}}>结束日期</th>
                  <th style={{...S.th,width:"17%"}}>备注</th>
                </tr></thead>
                <tbody>
                  {tlData.rows.map((r,i)=>(
              ...
              <div style={{marginTop:12,padding:"10px 12px",background:"#f8f5f0",borderRadius:6,fontSize:11,color:"#999",lineHeight:2}}>
                📌 招募周期 = max(0.5周, 总样本{params.sampleTotal}人 ÷ 60 × 难度系数{DIFF[params.difficulty].timeFactor}) = 建议 <b>{tlData.recruit.min}~{tlData.recruit.max}周</b> &nbsp;｜&nbsp;
                派发留置 = {params.weeksDuration}周×7 + 快递{params.courierDays}天 = <b>{tlData.leaveDays}天</b> &nbsp;｜&nbsp;
                Topline {params.products}款 → <b>{wkStr(tlData.toplineDays)}</b> &nbsp;｜&nbsp;
                Report {params.products}款 → <b>{wkStr(tlData.reportDays)}</b>
              </div>
            </div>
          )}
        </div>
...
const titleInputStyle = {background:"transparent",border:"none",borderBottom:"1px solid #5a504a",color:"#f5f2ed",fontSize:19,fontWeight:700,fontFamily:"inherit",outline:"none",width:360,padding:"2px 0"};
const S = {
